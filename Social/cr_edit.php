        ...
        <div class="content">
            ...
            <?php
                if(isset($_GET['page_id'])){
                    $get_id =$_GET['page_id'];
                    $get_page ="select * from pages where page_id='$get_id'";
                    $run_page = mysqli_query($con,$get_page);

                    $row = mysqli_fetch_array($run_page);
                    $page_con = $row['page_content'];
            ...
            <form action="" method="post" id="f">
                <h2>Edit Page </h2>
                <textarea cols="98" rows="5" name="page_content"  required="required"><?php echo $page_con;?></textarea><br/>
            ...
            <?php 
                if(isset($_POST['update'])){
                    ...
                    $update_page ="update pages set page_content='$content' where page_id='$get_id'";
                    $run_update = mysqli_query($con,$update_page);
                
                    if($run_update){
                        echo "<script>alert(' You edited this page ')</script>";
